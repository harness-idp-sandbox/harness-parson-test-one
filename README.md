...
# run app
java -jar target/parson-test-one-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t docker.io/parsontodd/parson-test-one:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/parson-test-one -n default

# get external endpoint
kubectl get svc parson-test-one-svc -n default
```
...
```bash
kubectl port-forward svc/parson-test-one-svc 8080:80 -n default
...
kubectl rollout restart deploy/parson-test-one -n default
kubectl rollout status deploy/parson-test-one -n default
```
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/parsontestone/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/parsontestone/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
